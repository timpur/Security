...
def create_dh_key():
    ...
    # Key range
    # 310 - 190 = 2^120 key range
    keyRangeStart = int(2**190) + 2
    keyRangeEnd = int(2**310) - 2

    # Generator used
    generator = 2

    # Private key, a random number between startt and end
    # Always a big random int. This is important for security
    privateKey = random.randint(keyRangeStart, keyRangeEnd)

    # Public key generated from private key and the huge prime
    # The big private key and large prime means best security for the key echange protocol
    publicKey = int(pow(generator,privateKey,prime))
    
    return (publicKey, privateKey) 

def calculate_dh_secret(their_public, my_private):
    # Calculate the shared secret
    # This is a verry secure key because of the large private key and prime used
    shared_secret = int(pow(their_public,my_private,prime))
